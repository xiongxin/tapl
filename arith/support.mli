...
module Error : sig
  (* An exception raised by the low-level error printer; exported
     ...
     exit status for the whole program.*)
  ...
  (* A conventient datatype for a "value with file info." Used in
     the lexer and parser. *)
  type 'a withinfo = {i: info; v: 'a}

  (* Print an error message and fail. The printing function is called
     in a context where the formatter is processing an hvbox. Insert calls to 
     Format.print_space to print a space or, if necessary, break the line
     at that point. *)
